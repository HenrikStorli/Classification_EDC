...
for k = 1:5
    % N1d: 1. indeks for training N2d: siste indeks for training, N1t: 1. indeks for test, N2t: siste indeks for test 
	N1d = (k-1)*10 +1; N2d = rem((k+2)*10-1, 50)+1; N1t = rem(N2d+1,50); N2t = rem(N2d+19,50)+1; 
	if(N2d < N1d)
        %I tilfelle indeksen "looper"
	...
	y1d =zeros(Ndtot,3); %Tom matrise
	y1d(:,1) = mvnpdf(x1d,x1m, x1s);
	y1d(:,2) = mvnpdf(x1d,x2m, x2s);
	y1d(:,3) = mvnpdf(x1d,x3m, x3s);
	[val1d,indd(:,1)] = max(y1d'); %Val1d = max values i max(y1dT), indd(:,1):legg indekser for maks i kolonne 1 av indd

	y2d =zeros(Ndtot,3); 
    %Lager normalfordeling for hver av klassene
	y2d(:,1) = mvnpdf(x2d,x1m, x1s);
	y2d(:,2) = mvnpdf(x2d,x2m, x2s);
	y2d(:,3) = mvnpdf(x2d,x3m, x3s);
	[val2d,indd(:,2)] = max(y2d'); %Fnutt = transpose
